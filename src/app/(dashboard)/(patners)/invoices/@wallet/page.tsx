...
export default function FacturePage(){
    ...
    //     const response = await api.get(`/transactions?transaction_receiver=${user._id}`);
    //     return response.data;
    // }
                ...
                <div className={'w-full flex gap-10 items-center'}>
                    {
                        isLoading
                            ...
                            <div className={'w-full flex gap-5'}>
                                <div className="w-full relative flex flex-col justify-center items-center bg-gradient-to-br from-custom_color-green to-emerald-700 rounded-2xl p-12 shadow-lg hover:shadow-xl transition-all duration-300">
                                    ...
                                    <span className={'text-6xl font-bold text-white flex gap-4 items-start'}>
                                        <sup className={'text-xl text-emerald-200 mt-2'}>{walletData && walletData?.currency}</sup>
                                ...
                                <div className={'flex gap-4 flex-col min-w-[200px]'}>
                                    <Button 
                                        variant={'outline'} 
                                        className={'flex-1 py-8 text-lg font-medium hover:bg-emerald-50 hover:text-custom_color-green hover:border-custom_color-green transition-colors duration-300 rounded-xl border-2'}
                                    >
                                    ...
                                    <Button 
                                        variant={'default'} 
                                        className={'flex-1 py-8 text-lg font-medium bg-custom_color-green hover:opacity-90 transition-colors duration-300 rounded-xl text-white'}
                                    >
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
